using Market.Application.Modules.Payments.Commands.ConfirmStripeSession;
...
namespace Market.API.Controllers
{
    ...
    public sealed class PaymentsController(ISender sender) : ControllerBase
    {
        ...
        [AllowAnonymous]
        [HttpPost("stripe/confirm-session")]
        public async Task<ActionResult<ConfirmStripeSessionResponse>> ConfirmStripeSession([FromBody] ConfirmStripeSessionRequest request, CancellationToken ct)
        {
            var result = await sender.Send(new ConfirmStripeSessionCommand(request.SessionId), ct);
            return Ok(result);
        }
    }
}
